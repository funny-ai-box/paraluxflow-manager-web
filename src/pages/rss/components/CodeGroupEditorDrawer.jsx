import {
  fetchRssGroupCrawlerScripts,
  publishRssGroupCrawlerScript,
  updateRssGroupCrawlerScript,
} from '@/services/rss';
...
export default function CodeEditorDrawer({ isVisible, onClose, groupId }) {
  ...
  const fetchScripts = async () => {
    setLoading(true);
    const result = await fetchRssGroupCrawlerScripts(groupId);
    setLoading(false);
    if (result['code'] === 200) {
      let fetchedScripts = result['data'];
      console.log(result);
      if (fetchedScripts.length === 0) {
        fetchedScripts = [{ group_id: groupId, script: initialCode.trim(), isNew: true }];
  ...
  const saveScript = async (script) => {
    ...
    const saveResult = await updateRssGroupCrawlerScript(script);
    return saveResult;
  };
    ...
    if (!isScriptChanged) {
      Modal.confirm({
        ...
        onOk: async () => {
          ...
          const publishResult = await publishRssGroupCrawlerScript({ group_id: groupId });
  ...
  return (
    <Drawer
      title={
        <div>
          <InfoCircleOutlined style={{ color: 'orange', marginRight: 10 }} />
          Edit Group Code(注意 这是当前feed所在group的脚本 修改后 整个group的脚本都会生效)
        </div>
      }
      placement="right"
      closable={true}
